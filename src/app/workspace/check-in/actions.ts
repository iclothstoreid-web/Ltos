...
export async function createConsultationSession(
  ...
  if (!user.data.user?.id) {
    return {
      ...
      customerConsultationToken: null,
    }
  }

  // consultation_number generated by DB via generate_consultation_number().
  // customer_consultation_token (Customer Self-Service Consultation Link)
  // generated the same way, via generate_customer_consultation_token() —
  // see migration 20260913000000_customer_consultation_self_service.sql.
  ...
  const { data, error } = await supabase
    ...
    .select('id, consultation_number, customer_consultation_token')
    .single()
  ...
  return {
    ...
    customerConsultationToken: data.customer_consultation_token,
  }
}