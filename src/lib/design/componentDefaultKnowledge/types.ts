...
//
// Shape mirrors AiDesignDna's own Component Knowledge fields
// (referenceInstruction/componentRules — see aiDna/types.ts) since this is
// the same kind of knowledge, one level up the inheritance chain.
// `referenceInstruction` is reserved for a future sprint: resolver.ts below
// already resolves it (delta wins, default is the fallback) so the shape is
// ready, but Recipe Composer does not consume that resolved value yet this
// sprint — only componentRules is wired into composeRenderRecipe (see
...
// from referenceInstruction/componentRules (never Hero Image, never prose
// reference instruction, never a Component Rule). Untyped
