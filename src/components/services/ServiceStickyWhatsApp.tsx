...
// Sprint W10 — always-on floating WhatsApp button, same pattern as
// CityStickyWhatsApp.tsx (distinct from the homepage-only, scroll-
// triggered StickyMobileCta.tsx, which links to /design-studio, not
// WhatsApp). Visible on both mobile and desktop per the brief's "sticky
// mobile CTA" + "floating consultation button" requirements.
export function ServiceStickyWhatsApp({ service, utm }: ServiceStickyWhatsAppProps) {
  ...
  return (
    <a
      ...
      onClick={() => trackCTA(`service_sticky_whatsapp_${service.slug}`, `/${service.slug}`, 'sticky_floating', 'landing')}
