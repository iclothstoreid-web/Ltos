...
export function ProductionSpecificationSheet({
  ...
  return (
    <div
      ...
      style={{ width }}
      className="fixed top-0 -left-[9999px] bg-white text-[#1b1b1c] p-10 font-sans"
    >
      <div className="flex items-center justify-between pb-5 border-b-2 border-[#1b1b1c]">
        <div>
          <p className="font-serif text-xl tracking-tight">LOCAL TAILOR</p>
          <p className="text-[9px] uppercase tracking-[0.25em] text-[#5a5a5a] mt-1">
            Production Specification Sheet
          </p>
        </div>
        <div className="text-right">
          <p className="text-[9px] uppercase tracking-widest text-[#5a5a5a]">Nomor Order</p>
          <p className="text-sm font-medium">{orderNumber}</p>
          <p className="text-[9px] uppercase tracking-widest text-[#5a5a5a] mt-2">Tanggal Produksi</p>
          <p className="text-sm font-medium">{today}</p>
        </div>
      </div>

      <div className="mt-5 mb-7">
        <p className="text-[9px] uppercase tracking-widest text-[#5a5a5a]">Nama Customer</p>
        <p className="text-xl font-serif mt-1">{customerName || '—'}</p>
      </div>

      {designRows.length > 0 && (
        <div className="mb-7">
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#5a5a5a] mb-3 pb-2 border-b border-[#1b1b1c]">
            Design Specification
          </p>
          <div className="grid grid-cols-2 gap-x-6 gap-y-3">
            {designRows.map(([label, value]) => (
              <div key={label}>
                <p className="text-[9px] uppercase tracking-widest text-[#5a5a5a]">{label}</p>
      ...
      <div className="mb-7">
        ...
        {!patternMeasurements ? (
          <p className="text-sm text-[#5a5a5a]">Belum ada data formulasi pola.</p>
        ) : (
          <>
            {template && (
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-2">
              {(Object.keys(FIELD_LABELS) as Array<MeasurementKey>).map(key => (
                <div key={key} className="flex items-baseline justify-between border-b border-[#e5e5e5] pb-1">
                  <span className="text-xs text-[#5a5a5a]">{FIELD_LABELS[key]}</span>
      ...
      {customerPhotoUrl && (
        <div>
          ...
          <div>
            <p className="text-[9px] uppercase tracking-widest text-[#5a5a5a] mb-2">Foto Customer</p>
            ...
            <img
              ...
              alt="Foto Customer"
              className="w-40 h-40 object-cover rounded border border-[#c6c6cc]"
