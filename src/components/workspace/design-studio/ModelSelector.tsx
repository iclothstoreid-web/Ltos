...
export function ModelSelector({ options, selected, onSelect, onViewSpec }: ModelSelectorProps) {
  return (
    <div className="space-y-3">
      {options.length === 0 && <EmptyOptionsState label="Model Busana" />}
      <div className="grid grid-cols-2 gap-4">
        <button
          type="button"
          onClick={() => onSelect(NONE_SELECTION)}
          className={`border-[0.5px] p-2 transition-all ${
            isNoneSelection(selected) ? 'border-[#775a19] bg-[#775a19]/5' : 'border-[#c4c7c7]'
          }`}
        >
          <div className="h-40 bg-[#dce2f3] mb-2 flex items-center justify-center">
            <span className="material-symbols-outlined text-5xl text-[#775a19]/40">block</span>
          </div>
          <span className="font-sans text-sm block text-center text-[#151c27]">(None)</span>
        </button>
        {options.map(option => {
