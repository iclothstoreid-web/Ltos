import { deliveryCopy } from '@/lib/marketing/copy'
  ...
  return (
    <section id="delivery" aria-labelledby="delivery-heading" className="bg-luxury-navy-deep px-6 py-24 md:px-10">
      <div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-2 lg:items-center">
        <Reveal>
          <article aria-label={deliveryCopy.experienceLabel} className="rounded-sm border border-luxury-gold/30 bg-luxury-charcoal/60 p-6 md:p-8">
            <p className="font-luxury-sans text-xs uppercase tracking-[0.2em] text-luxury-gold">{deliveryCopy.experienceLabel}</p>
            <ul className="mt-5 flex flex-col gap-4">
              {deliveryCopy.experience.map((item) => (
        ...
        <Reveal delay={0.1}>
          ...
          <p className="font-luxury-sans text-xs uppercase tracking-[0.2em] text-luxury-gold">{deliveryCopy.eyebrow}</p>
          <h2 id="delivery-heading" className="mt-3 font-fraunces text-3xl text-luxury-ivory md:text-4xl">
            {deliveryCopy.heading}
          </h2>
          <p className="mt-4 max-w-md font-luxury-sans text-sm text-luxury-taupe">{deliveryCopy.subheadline}</p>

          <blockquote className="mt-8 border-l-2 border-luxury-gold/40 pl-5">
            <p className="font-fraunces text-xl italic leading-snug text-luxury-ivory md:text-2xl">&ldquo;{deliveryCopy.statement}&rdquo;</p>
          </blockquote>
