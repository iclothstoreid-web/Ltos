...
export function FabricHighlight() {
  const t = useTranslations('home.fabric')
  const cardTraits = t.raw('cards') as { origin: string; trait: string }[]
        ...
        <ul className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {fabricCopy.cards.map((card, i) => (
            <Reveal as="li" key={card.name} delay={i * 0.08}>
              ...
              <p className="font-luxury-sans text-xs text-luxury-taupe">{cardTraits[i]?.origin ?? card.origin}</p>
              <p className="mt-1 font-luxury-sans text-xs text-luxury-taupe">{cardTraits[i]?.trait ?? card.trait}</p>
            </Reveal>
          ))}
        </ul>
