import { productionCopy } from '@/lib/marketing/copy'
    ...
    <section id="production" aria-labelledby="production-heading" className="bg-luxury-navy-deep px-6 py-24 md:px-10">
      <div className="mx-auto max-w-6xl">
        <Reveal className="mx-auto max-w-2xl text-center">
          ...
          <p className="font-luxury-sans text-xs uppercase tracking-[0.2em] text-luxury-gold">{productionCopy.eyebrow}</p>
          <h2 id="production-heading" className="mt-3 font-fraunces text-3xl text-luxury-ivory md:text-4xl">
            {productionCopy.heading}
          </h2>
          <p className="mt-4 font-luxury-sans text-sm text-luxury-taupe">{productionCopy.subheadline}</p>
        </Reveal>

        <ol className="mt-14 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
          {productionCopy.stages.map((stage, i) => (
        ...
        <Reveal delay={0.15} className="mt-16 border-y border-luxury-gold/[0.14] py-10">
          <ul className="grid grid-cols-2 gap-8 lg:grid-cols-4">
            {productionCopy.metrics.map((metric) => (
        ...
        <Reveal delay={0.2} className="mt-14 text-center">
          <blockquote className="mx-auto max-w-xl border-l-2 border-luxury-gold/40 pl-5 text-left">
            <p className="font-fraunces text-xl italic leading-snug text-luxury-ivory md:text-2xl">&ldquo;{productionCopy.statement}&rdquo;</p>
          </blockquote>
